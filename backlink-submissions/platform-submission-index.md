...
这些文件只准备提交内容，不代表平台已经发布外链。账号登录、邮箱验证、CAPTCHA、2FA、管理员审核和最终提交由账号持有人完成。
...
每个完成的平台都要记录：公开 URL、根域、目标 URL、实际可见锚文本、链接位置、`rel` tokens、HTTP 状态、页面级 `noindex`/`X-Robots-Tag`、发布日期和下次复查日期。只有公开详情页验证通过后，才把状态从 `Prepared` 或 `Pending` 改为 `Complete`。
